...
namespace Easy3D
{
    ...
    namespace
    {
        std::vector<VertexPositionTexture> ToVertexPositionTexture(const std::vector<BillboardVertex>& vertices)
        {
    ...
    BillboardMeshRenderer::BillboardMeshRenderer(GraphicsDevice& device,
          ...
          m_primitiveCount(static_cast<int>(indices.size() / 3))
    {
        if (indices.size() % 3 != 0)
        {
            throw std::invalid_argument("BillboardMeshRenderer: index count must be a multiple of 3");
        }
    ...
    void BillboardMeshRenderer::Draw(GraphicsDevice& device, BasicEffect& effect) const
    {
